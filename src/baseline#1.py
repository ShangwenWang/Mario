...
def main(testFieldMethods, baseFieldMethods, fieldDcit, outputPath):
    ...
    for i, fileHook in enumerate(tqdm(fileGraph)):
        for curIdx, targetCls in enumerate(fileGraph[fileHook]):
            ...
            curMax = ['', 0]  # The most similar class, the semantic "similarity"
            for ctxIdx, ctxCls in enumerate(fileGraph[fileHook]):
                if ctxIdx == curIdx or ctxCls not in baseFieldMethods or 'main' in os.path.basename(ctxCls).lower() or\
                    ...
                    continue
            if targetCls in fieldDcit:
                ctxMethods = ['get' + x.capitalize() for x in fieldDcit[targetCls]]
                ctxMethods += ['set' + x.capitalize() for x in fieldDcit[targetCls]]
            else:
                ctxMethods = []
            res[targetCls] = [testFieldMethods[targetCls], ctxMethods]
...
'''
This baseline always recommends get and set.
'''
if __name__ == '__main__':
    ...
    outputPath = '../Data/Baseline#1.json'
    fieldDcitPath = '../Data/fieldDict.pkl'
    if len(sys.argv) > 1:
        threshold = float(sys.argv[1])
    else:
        threshold = 0.1
    with open(testFieldIrMethodsPath, 'rb') as f_1, open(baseFieldIrMethodsPath, 'rb') as f_2, \
            open(testFieldReMethodsPath, 'rb') as f_3, open(baseFieldReMethodsPath, 'rb') as f_4,   open(fieldDcitPath, 'rb') as f_5:
        ...
        fieldDcit = pickle.load(f_5)


    testFieldMethods = combimeMethods(testFieldIrMethods, testFieldReMethods)
    baseFieldMethods = baseFieldReMethods

    main(testFieldMethods, baseFieldMethods, fieldDcit, outputPath)
